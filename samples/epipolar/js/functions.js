...
var GeometryEditor = {
	...
	editorActive : false, 
	/** */
	epipolarCursors : new THREE.Object3D(),
	helpersVisible : true,
	 /** temp var,  
	...
	init : function () {
		...
		scene.add( GeometryEditor.epipolarCursors );
		...
		else if (GeometryEditor.createScene != undefined) {
			views = GeometryEditor.createScene;
		}
		else {
			views.push({
				left: 0,
		...
		for (var i =  0; i < views.length; i++ ) {
			...
			if(i === 0) {
				...
				camera.helper.traverse( function ( object ) { 
		...
		if(GeometryEditor.createRandom) {
			var geom = new THREE.BoxGeometry( 2, 2, 2 );
			var range = 5;
			for(var i = 0; i < 50; i++ ) {
		        var grayness = Math.random() - 0.25,
		                mat = new THREE.MeshLambertMaterial(),
		                cube = new THREE.Mesh( geom, mat );
		        mat.color.setRGB( grayness, grayness, grayness );
		        cube.position.set( range * (0.5 - Math.random()), range * (0.5 - Math.random()), range * (0.5 - Math.random()) );
		        cube.rotation.set( Math.random(), 
		        	Math.random(), 
		        	Math.random() );
		        cube.primalColor = (grayness.toString(16)); // *** NOTE THIS
		        
		        sceneGeometry.add( cube );
			}
		}

		// the epipole function
		document.oncontextmenu = function() {
			if(GeometryEditor.pickerActive && !GeometryEditor.editorActive) {
				geometry = new THREE.SphereGeometry( 0.2, 10, 10);
				var material = new THREE.MeshBasicMaterial({ color: 0xFF0000 });
				var sphere = new THREE.Mesh( geometry, material );
				GeometryEditor.epipolarCursors.add( sphere ); // and add to scene
				GeometryEditor.lastEpipolarCursor = sphere;

				sphere.position = GeometryEditor.raycastingCursor.position;
				var main = sphere.position.clone();
				console.log("main | x:"+ main.x + ", y=" + main.y + ", z="+ main.z);
				...
				var pointCam1 = main.clone();
				projector.projectVector( pointCam1, views[1].camera );
				pointCam1.x = ( pointCam1.x * widthHalf ) + widthHalf;
				pointCam1.y = - ( pointCam1.y * heightHalf ) + heightHalf;
				console.log("proj cam 1 | x="+ pointCam1.x +",y="+ pointCam1.y);

				/*$("<div class='marker'></div>")
				.appendTo("body")
				.css({
					top:pointCam1.y-divSize,
					left:pointCam1.x+width-divSize
				});*/


				var pointCam2 = main.clone();
				projector.projectVector( pointCam2, views[2].camera );
				pointCam2.x = ( pointCam2.x * widthHalf ) + widthHalf;
				pointCam2.y = - ( pointCam2.y * heightHalf ) + heightHalf;
				console.log("proj cam 2 | x="+ pointCam2.x +",y="+ pointCam2.y);

				/*$("<div class='marker'></div>")
				.appendTo("body")
				.css({
					top:pointCam2.y+height-divSize,
					left:pointCam2.x+width-divSize
				});*/


				var epipole1 = views[1].camera.position.clone();
				projector.projectVector( epipole1, views[2].camera );
				epipole1.x = ( epipole1.x * widthHalf ) + widthHalf;
				epipole1.y = - ( epipole1.y * heightHalf ) + heightHalf;
				console.log("proj cam 2 | x="+ epipole1.x +",y="+ epipole1.y);

				/*$("<div class='marker'>cam1</div>")
				.appendTo("body")
				.css({
					top:epipole1.y+height-divSize,
					left:epipole1.x+width-divSize
				});*/

				var epipole2 = views[2].camera.position.clone();
				projector.projectVector( epipole2, views[1].camera );
				epipole2.x = ( epipole2.x * widthHalf ) + widthHalf;
				epipole2.y = - ( epipole2.y * heightHalf ) + heightHalf;
				console.log("proj cam 2 | x="+ epipole2.x +",y="+ epipole2.y);

				/*$("<div class='marker'>cam2</div>")
				.appendTo("body")
				.css({
					top:epipole2.y-divSize,
					left:epipole2.x+width-divSize
				});*/


				var context = document.getElementById('drawSpace2').getContext('2d');

				var direction = vec3(
					-pointCam2.x+epipole1.x,
					-pointCam2.y+epipole1.y,
					0);

				context.beginPath();
				context.strokeStyle = "#f00";
				context.moveTo(pointCam2.x-direction.x*100, pointCam2.y-direction.y*100);
				context.lineTo(epipole1.x+direction.x*100, epipole1.y+direction.y*100);
				context.lineWidth = 1;
				context.stroke();
				context.closePath();

				context = document.getElementById('drawSpace1').getContext('2d');
				direction = vec3(
					-pointCam1.x+epipole2.x,
					-pointCam1.y+epipole2.y,
					0);

				context.beginPath();
				context.strokeStyle = "#f00";
				context.moveTo(pointCam1.x-direction.x*100, pointCam1.y-direction.y*100);
				context.lineTo(epipole2.x+direction.x*100, epipole2.y+direction.y*100);
				context.lineWidth = 1;
				context.stroke();
				context.closePath();
				...
				context.moveTo(pointCam1.x-3,pointCam1.y-3);
				context.lineTo(pointCam1.x+3,pointCam1.y+3);
				context.moveTo(pointCam1.x+3,pointCam1.y-3);
				context.lineTo(pointCam1.x-3,pointCam1.y+3);
				context.stroke();
	...
	/**
	 * Clears the epipolar scene
	 */
	clearEpipolar : function () {
		// remove markers
		$(".marker").remove();
		// in three.js cursors
		scene.remove(GeometryEditor.epipolarCursors);
		GeometryEditor.epipolarCursors = new THREE.Object3D();
		scene.add(GeometryEditor.epipolarCursors);

		document.getElementById('drawSpace1').width = document.getElementById('drawSpace1').width;
		document.getElementById('drawSpace2').width = document.getElementById('drawSpace2').width;
	},
	...
	render : function () {
		...
		if ( windowWidth != window.innerWidth || windowHeight != window.innerHeight ) {
			...
			views[1].camera.helper.update();
			views[2].camera.helper.update();
			
			GeometryEditor.clearEpipolar();
	...
	onKeyDown : function (event) {
		...
		if(!GeometryEditor.inputFocused)
			switch( event.keyCode ) {
				...
				case 187: // +
					//move last point
					//console.log(GeometryEditor.lastEpipolarCursor.position.x + ", " +GeometryEditor.lastEpipolarCursor.position.y + ", " + GeometryEditor.lastEpipolarCursor.position.z);
					var cam = views[1].camera.position.clone();
					var point = GeometryEditor.lastEpipolarCursor.position.clone();
					var camToPoint = vec3(point.x-cam.x, point.y-cam.y, point.z-cam.z).normalize().multiplyScalar(0.3);
					//console.log(camToPoint.x + ", " +camToPoint.y + ", " + camToPoint.z);

					GeometryEditor.lastEpipolarCursor.position = vec3(parseFloat(GeometryEditor.lastEpipolarCursor.position.x)+ parseFloat(camToPoint.x),
						parseFloat(GeometryEditor.lastEpipolarCursor.position.y)+ parseFloat(camToPoint.y),
						parseFloat(GeometryEditor.lastEpipolarCursor.position.z)+ parseFloat(camToPoint.z));
					//console.log(GeometryEditor.lastEpipolarCursor.position.x + ", " +GeometryEditor.lastEpipolarCursor.position.y + ", " + GeometryEditor.lastEpipolarCursor.position.z);
				...
				case 189: // - 
					...
					var point = GeometryEditor.lastEpipolarCursor.position.clone();
					var camToPoint = vec3(point.x-cam.x, point.y-cam.y, point.z-cam.z).normalize().multiplyScalar(0.3);
					//camToPoint.subVectors(point,cam);
					//console.log(camToPoint.x + ", " +camToPoint.y + ", " + camToPoint.z);
				...
				case 13: // enter
					if(GeometryEditor.helpersVisible) {
						views[1].camera.helper.visible = false;
						views[2].camera.helper.visible = false;
						/*THREE.SceneUtils.traverseHierarchy( views[1].camera.helper, function ( object ) { object.visible = false; } );
						THREE.SceneUtils.traverseHierarchy( views[2].camera.helper, function ( object ) { object.visible = false; } );*/
						GeometryEditor.helpersVisible = false;
					}
					else{
						views[1].camera.helper.visible = true;
						views[2].camera.helper.visible = true;
						/*THREE.SceneUtils.traverseHierarchy( views[1].camera.helper, function ( object ) { object.visible = true; } );
						THREE.SceneUtils.traverseHierarchy( views[2].camera.helper, function ( object ) { object.visible = true; } );*/
						GeometryEditor.helpersVisible = true;
					}
					break;
			}
